...
function AES_Decode(base64_text)
{
	try {
		...
		let domain = hostname.substring(hostname.indexOf("."), hostname.length);
		// 암호화키 만료된 경우
		sessionStorage.removeItem("_newSecretKey");
		document.cookie = "new_secret_key='';expires=" + new Date().toUTCString() + ';path=/; Domain=' + domain;
		location.reload();
	}
}
